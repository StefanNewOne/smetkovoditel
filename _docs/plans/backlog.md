...
| SM-2 | Maintenance | Commit toolchain: Husky + commitlint + lint-staged + gitleaks + commitizen | — | ☐ |
| SM-3 | Maintenance | Docker Compose (Postgres/app/worker/MinIO/Mailhog/Nginx) + `.env.example` | §11 | ☐ |
| SM-4 | Feature | Prisma schema transcribed literally from Master Plan ДЕЛ II + first migration | ДЕЛ II | ☐ |
| SM-5 | Feature | Session auth + RBAC + append-only AuditLog middleware | §9.8 | ☐ |
| SM-6 | Feature | App shell + design tokens (sidebar, header, Manrope, period indicator) | handoff | ☐ |
| SM-7 | Maintenance | `scripts/deploy.sh` + `docker-compose.prod.yml` + Nginx/Certbot deployment docs | §11 | ☐ |
...
| SM-15 | Feature | Invoice PDF template with Cyrillic font (§7, D1) | §7, D1 | ☐ |
...
| SM-38 | Testing | Golden-file fixtures (statements 146/149 + 3 Meta invoices) + T1–T8, T12 | §13, T1–T8 | ☐ |
